...
$price = $_GET['total'];
    ...
    if(isset($_POST['pay'])){
       $stu_id = $_POST['student_id'];
        ...
        $select = $conn->query("SELECT * FROM cart WHERE byid = '$stu_id' AND status = 'pending'");
          ...
          if ($count > 0) {
            foreach ($fetch as $value) {
                ...
                $goods = str_replace("temp", "a", $goods);
                $goods = str_replace("pay first", "Downlnload", $goods);

                $inner = array('goods' => $goods, );

                update('cart',$inner,'byid',$stu_id);
            }
        }
        ...
        $to_email= $identity;
        $to_name=$identity;
        $subject='Aknowledgement of Priduct Purchase';
        $body ='<html><body style="text-align:center;">';
        $body.=' <div style="border:1px solid #e5e5e5; border-radius:5px; margin:20px;">';
        $body.=' <div style="margin:10px;">';
        $body.=' <p style="text-align:justify;">Dear '.$identity.'</p>';
        $body.=' <p style="text-align:justify;">NGN'.$amount.' have been successfully paid for '.$goods.'</p>';
        $body.=' <p style="text-align:justify;">You can now login to download it</p>';
        $body.=' <p style="text-align:justify;">Thank you for choosing Poly Bookstore</p>';
        $body.=' <br>';
        $body.=' </div>';
        $body.=' </div>';
        $body.='</body></html>';
...
<head>
    ...
    <title>WebPAY - POLY BOOKSTORE CART PAYMENT</title>
        ...
        <div id="page-logo-header" class="">
            <div id="page-logo-header-content">
                ...
                <div id="page-title-header" class="">
                    <div class="merchant_name" style="display: "><p> POLY BOOKSTORE CART PAYMENT </p></div>
